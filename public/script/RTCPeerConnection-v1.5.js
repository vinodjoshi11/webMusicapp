...
function RTCPeerConnection(options) {
    ...
    if (!moz && chromeVersion < 28) { alert(11);
        iceServers.push({
            url: 'turn:homeo@turn.bistri.com:3000',
            credential: 'homeo'
        });
    }
...
function getUserMedia(options) {
    ...
    function streaming(stream) {
        var playlist=localStorage.getItem('liveStrem')
        console.log(playlist);
        var video = options.video;
        if (video) {
            if($('#player').hasClass('icon-play')){
                console.log('pause')
               // video[moz ? 'mozSrcObject' : 'src']=moz ? stream :playlist;
                video.pause();
            }
            else if ( $('#player').hasClass("icon-pause")){
                console.log('play')
              //  video[moz ? 'mozSrcObject' : 'src']=moz ? stream :playlist;
                 video.play();
